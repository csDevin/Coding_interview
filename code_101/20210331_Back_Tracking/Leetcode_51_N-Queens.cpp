// L51: N-Queens (Hard)
// 时间复杂度：时间复杂度：O(N!)，其中N是皇后数量，第0行N个选择，第1行最多N-1个选择，第2行最多N-2个选择，因为皇后不能在同一列。
// 空间复杂度：空间复杂度：O(N)，空间复杂度主要取决于递归调用层数、记录每行放置的皇后的列下标的数组以及三个集合，
// 递归调用层数不会超过 N，数组的长度为 N，每个集合的元素个数都不会超过N。
// vector<string> temp(n, string(n, '.'));视作n个string，空间复杂度O(N),vector<vector<string>> result,包含
// 常熟k个n个string，空间复杂度K*O(N),忽略常数。


// 回溯法（本质上是排列枚举）
// 排列枚举：因为这里每行只能放置一个皇后，而所有行中皇后的列号正好构成一个1到n的排列，所以我们可以把问题转化为一个排列枚举，规模是n!。
